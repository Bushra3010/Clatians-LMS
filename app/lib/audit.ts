 ...
 */
export async function logAudit(actor: Pick<User, "id" | "name" | "role"> | null, action: string, detail = ""): Promise<void> {
  try {
    await db.prepare(
      "INSERT INTO audit_log (id, actor_id, actor_name, actor_role, action, detail) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(newId(), actor?.id ?? null, actor?.name ?? "System", actor?.role ?? "", action, detail);
  } catch {
